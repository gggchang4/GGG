...
本项目不是传统技术博客，也不是普通简历页，而是一个兼具视觉审美、前端交互能力、动效实现能力和工程化展示价值的个人 Profile 网站。
...
网站整体不应做成普通博客模板，而应更接近一个小型产品官网、创意作品集或交互实验场。

## 2. 核心目标

第一版网站的核心目标不是功能多，而是让访问者在 5 秒内形成清晰印象：
...
这个网站的目标不是以文章输出为主，而是展示个人审美和前端交互能力。因此技术重点应放在：
...
但本项目最终选用更适合国际化创意前端、交互展示和工程表达的 React 方案：
...
第一版不建议加入后端和数据库，应保持轻量、静态优先、内容本地化。
...
`/works` 用于展示真实项目和 Case Study。  
`/lab` 用于展示交互实验、动效能力和 WebGL / 3D 技术能力。

## 6. 页面内容规划

### 6.1 Home 首页

首页承担第一印象，不应只是普通头像和文字介绍。
...
作品集页面不应只放项目截图，建议将重点项目做成 Case Study。
...
### 7.1 高级科技感 / AI Agent 风格

适合展示 AI、Agent 和前端工程方向。
...
| 优先级 | 内容 | 目标 |
| --- | --- | --- |
| P0 | Home Hero | 建立第一印象和个人品牌 |
| P0 | Works 精选项目 | 展示真实项目能力 |
| P0 | Lab 单个交互 Demo | 展示差异化交互能力 |
| P1 | About / Stack | 补充个人背景和技术能力 |
| P1 | Contact | 提供明确联系入口 |
| P2 | Case Study 详情页 | 后续强化项目深度 |
| P2 | 多个 Lab 实验 | 后续扩展创意前端能力 |
